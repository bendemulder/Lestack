...
def process_video(input_path, output_path, pstack, resample_factor, drizzle_factor, 
                  ...
                  stack_method, blurw=1, contrastw=1, ssimw=1, gradientw=1, entropyw=1):
    ...
    def compute_scores(frame, reference_frame):
        ...
        blur_value_norm = (blur_value - min_blur) / (max_blur - min_blur)
        contrast_value_norm = (contrast_value - min_contrast) / (max_contrast - min_contrast)
        ssim_value_norm = (ssim_value - min_ssim) / (max_ssim - min_ssim)
        gradient_magnitude_norm = (gradient_magnitude - min_gradient) / (max_gradient - min_gradient)
        entropy_value_norm = (entropy_value - min_entropy) / (max_entropy - min_entropy)
    ...
    with ThreadPoolExecutor(max_workers=ncore) as executor:
        futures = [executor.submit(compute_blur_value, frame) for frame in sample_frames]
        blur_values = [future.result() for future in tqdm(as_completed(futures), total=len(futures), desc="Computing blur values", disable=quiet)]
        min_blur, max_blur = min(blur_values), max(blur_values)

        futures = [executor.submit(compute_contrast_value, frame) for frame in sample_frames]
        contrast_values = [future.result() for future in tqdm(as_completed(futures), total=len(futures), desc="Computing contrast values", disable=quiet)]
        min_contrast, max_contrast = min(contrast_values), max(contrast_values)
        
        futures = [executor.submit(compute_gradient_magnitude, frame) for frame in sample_frames]
        gradient_magnitudes = [future.result() for future in tqdm(as_completed(futures), total=len(futures), desc="Computing gradient magnitudes", disable=quiet)]
        min_gradient, max_gradient = min(gradient_magnitudes), max(gradient_magnitudes)
        
        futures = [executor.submit(compute_entropy, frame) for frame in sample_frames]
        entropies = [future.result() for future in tqdm(as_completed(futures), total=len(futures), desc="Computing entropies", disable=quiet)]
        min_entropy, max_entropy = min(entropies), max(entropies)

    # SSIM is already normalized between 0 and 1, so no need to calculate min/max
    min_ssim, max_ssim = 0, 1

    reference_frame = frames[np.argmin(blur_values)]  # Use the least blurry frame as reference

    with ThreadPoolExecutor(max_workers=ncore) as executor:
        future_to_frame = {executor.submit(compute_scores, frame, reference_frame): frame for frame in frames}
        frame_scores = [future.result() for future in tqdm(as_completed(future_to_frame), total=len(future_to_frame), desc="Computing frame scores", disable=quiet)]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Process video frames, align, and stack them to generate a final image. (Version 0.2.0)")
    ...
    parser.add_argument('-blurw', type=float, default=1.0, help='Weight for blur value')
    parser.add_argument('-contrastw', type=float, default=1.0, help='Weight for contrast value')
    parser.add_argument('-ssimw', type=float, default=1.0, help='Weight for SSIM value')
    parser.add_argument('-gradientw', type=float, default=1.0, help='Weight for gradient magnitude')
    parser.add_argument('-entropyw', type=float, default=1.0, help='Weight for entropy value')
